...
const SCENARIOS: Scenario[] = [
  { name: 'Knight-Outpost-Case', fen: '6k1/p6p/8/3N4/4P3/8/8/4K3 w - - 0 1' },
...
const C1_C21_C22_C23_OVERRIDES: Partial<AiTuning> = {
  ...C1_C21_C22_OVERRIDES,
  enableKnightOutpost: true,
};

const CONFIGS = [
  { name: 'c1+c2.1+c2.2', overrides: C1_C21_C22_OVERRIDES },
  { name: 'c1+c2.1+c2.2+c2.3', overrides: C1_C21_C22_C23_OVERRIDES },
] as const;
...
const main = (): void => {
  ...
  const base = summary['c1+c2.1+c2.2'];
  const c23 = summary['c1+c2.1+c2.2+c2.3'];

  const rows = [
    ['Metric', 'C1+C2.1+C2.2', 'C1+C2.1+C2.2+C2.3', 'Delta (C2.3 vs C2.2)'],
    ['Samples', String(base.samples), String(c23.samples), '-'],
    ['Avg ms', base.avgMs.toFixed(2), c23.avgMs.toFixed(2), pct(base.avgMs, c23.avgMs)],
    ['Median ms', base.medianMs.toFixed(2), c23.medianMs.toFixed(2), pct(base.medianMs, c23.medianMs)],
    ['Avg nodes', base.avgNodes.toFixed(0), c23.avgNodes.toFixed(0), pct(base.avgNodes, c23.avgNodes)],
    ['Avg qNodes', base.avgQNodes.toFixed(0), c23.avgQNodes.toFixed(0), pct(base.avgQNodes, c23.avgQNodes)],
    ['qNode ratio', base.qNodeRatio.toFixed(4), c23.qNodeRatio.toFixed(4), pct(base.qNodeRatio, c23.qNodeRatio)],
    ['Avg depth', base.avgDepth.toFixed(2), c23.avgDepth.toFixed(2), pct(base.avgDepth, c23.avgDepth)],
  ];
  ...
  const outFile = path.join(outDir, 'phaseC23-benchmark-2026-03-03.md');
