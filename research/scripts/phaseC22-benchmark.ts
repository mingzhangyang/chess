...
const SCENARIOS: Scenario[] = [
  { name: 'Backward-Pawn-Case', fen: '6k1/8/8/8/4pp2/3P4/8/4K3 w - - 0 1' },
...
const main = (): void => {
  ...
  const base = summary['c1+c2.1'];
  const c22 = summary['c1+c2.1+c2.2'];

  const rows = [
    ['Metric', 'C1+C2.1', 'C1+C2.1+C2.2', 'Delta (C2.2 vs C1+C2.1)'],
    ['Samples', String(base.samples), String(c22.samples), '-'],
    ['Avg ms', base.avgMs.toFixed(2), c22.avgMs.toFixed(2), pct(base.avgMs, c22.avgMs)],
    ['Median ms', base.medianMs.toFixed(2), c22.medianMs.toFixed(2), pct(base.medianMs, c22.medianMs)],
    ['Avg nodes', base.avgNodes.toFixed(0), c22.avgNodes.toFixed(0), pct(base.avgNodes, c22.avgNodes)],
    ['Avg qNodes', base.avgQNodes.toFixed(0), c22.avgQNodes.toFixed(0), pct(base.avgQNodes, c22.avgQNodes)],
    ['qNode ratio', base.qNodeRatio.toFixed(4), c22.qNodeRatio.toFixed(4), pct(base.qNodeRatio, c22.qNodeRatio)],
    ['Avg depth', base.avgDepth.toFixed(2), c22.avgDepth.toFixed(2), pct(base.avgDepth, c22.avgDepth)],
  ];
  ...
  const outFile = path.join(outDir, 'phaseC22-benchmark-2026-03-03.md');
