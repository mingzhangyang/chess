...
const SCENARIOS: Scenario[] = [
  { name: 'Passed-Pawn-King-Distance', fen: '7k/2K5/3P4/8/8/8/8/8 w - - 0 1' },
...
const C1_C21_C22_C23_C24_OVERRIDES: Partial<AiTuning> = {
  ...C1_C21_C22_OVERRIDES,
  enablePassedPawnKingDistance: true,
};

const CONFIGS = [
  { name: 'c1+c2.1+c2.2+c2.3', overrides: C1_C21_C22_OVERRIDES },
  { name: 'c1+c2.1+c2.2+c2.3+c2.4', overrides: C1_C21_C22_C23_C24_OVERRIDES },
] as const;
...
const main = (): void => {
  ...
  const base = summary['c1+c2.1+c2.2+c2.3'];
  const c24 = summary['c1+c2.1+c2.2+c2.3+c2.4'];

  const rows = [
    ['Metric', 'C1+C2.1+C2.2+C2.3', 'C1+C2.1+C2.2+C2.3+C2.4', 'Delta (C2.4 vs C2.3)'],
    ['Samples', String(base.samples), String(c24.samples), '-'],
    ['Avg ms', base.avgMs.toFixed(2), c24.avgMs.toFixed(2), pct(base.avgMs, c24.avgMs)],
    ['Median ms', base.medianMs.toFixed(2), c24.medianMs.toFixed(2), pct(base.medianMs, c24.medianMs)],
    ['Avg nodes', base.avgNodes.toFixed(0), c24.avgNodes.toFixed(0), pct(base.avgNodes, c24.avgNodes)],
    ['Avg qNodes', base.avgQNodes.toFixed(0), c24.avgQNodes.toFixed(0), pct(base.avgQNodes, c24.avgQNodes)],
    ['qNode ratio', base.qNodeRatio.toFixed(4), c24.qNodeRatio.toFixed(4), pct(base.qNodeRatio, c24.qNodeRatio)],
    ['Avg depth', base.avgDepth.toFixed(2), c24.avgDepth.toFixed(2), pct(base.avgDepth, c24.avgDepth)],
  ];
  ...
  const outFile = path.join(outDir, 'phaseC24-benchmark-2026-03-03.md');
