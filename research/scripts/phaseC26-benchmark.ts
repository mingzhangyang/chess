...
const SCENARIOS: Scenario[] = [
  { name: 'Tempo-Case', fen: '6k1/8/8/8/3P4/8/8/4K3 w - - 0 1' },
...
const C1_C21_C22_C23_C24_C25_C26_OVERRIDES: Partial<AiTuning> = {
  ...C1_C21_C22_OVERRIDES,
  enableTempoBonus: true,
};

const CONFIGS = [
  { name: 'c1+c2.1+c2.2+c2.3+c2.4+c2.5', overrides: C1_C21_C22_OVERRIDES },
  { name: 'c1+c2.1+c2.2+c2.3+c2.4+c2.5+c2.6', overrides: C1_C21_C22_C23_C24_C25_C26_OVERRIDES },
] as const;
...
const main = (): void => {
  ...
  const base = summary['c1+c2.1+c2.2+c2.3+c2.4+c2.5'];
  const c26 = summary['c1+c2.1+c2.2+c2.3+c2.4+c2.5+c2.6'];

  const rows = [
    ['Metric', 'C1+C2.1+C2.2+C2.3+C2.4+C2.5', 'C1+C2.1+C2.2+C2.3+C2.4+C2.5+C2.6', 'Delta (C2.6 vs C2.5)'],
    ['Samples', String(base.samples), String(c26.samples), '-'],
    ['Avg ms', base.avgMs.toFixed(2), c26.avgMs.toFixed(2), pct(base.avgMs, c26.avgMs)],
    ['Median ms', base.medianMs.toFixed(2), c26.medianMs.toFixed(2), pct(base.medianMs, c26.medianMs)],
    ['Avg nodes', base.avgNodes.toFixed(0), c26.avgNodes.toFixed(0), pct(base.avgNodes, c26.avgNodes)],
    ['Avg qNodes', base.avgQNodes.toFixed(0), c26.avgQNodes.toFixed(0), pct(base.avgQNodes, c26.avgQNodes)],
    ['qNode ratio', base.qNodeRatio.toFixed(4), c26.qNodeRatio.toFixed(4), pct(base.qNodeRatio, c26.qNodeRatio)],
    ['Avg depth', base.avgDepth.toFixed(2), c26.avgDepth.toFixed(2), pct(base.avgDepth, c26.avgDepth)],
  ];
  ...
  const outFile = path.join(outDir, 'phaseC26-benchmark-2026-03-03.md');
